...
class WeatherClient:
    def __init__(self, latitude, longitude, timezone=None):
        ...
        self.date = datetime.datetime.now(pytz.timezone("America/Los_Angeles")).date().strftime('%Y-%m-%d')
    ...
    def sunset(self):
        return datetime.datetime.utcfromtimestamp(self.data["current"]["sunset"]).replace(
            tzinfo=pytz.utc
        )
